...
from src.evaluation_scripts.evaluate_single_test_file import run_evaluation
...
def incremental_autoencoder_training(activity_buffers, autoencoders, scalers, models_dir,
    ...
    if is_warmup_phase:
        ...
        cluster_reg_config = None
        memory_config = {"enable_memory_regularization": False}
...
def process_event_stream(file_path, activity_embeddings, online_onehot_encoders,
    ...
    checkpoint_interval = training_mode_config.get("checkpoint_interval")
    inference_event_counter = 0
    event_processing_errors = 0
    _reported_first_event_error = False
    event_vectors_buffer = []
    ...
    activity_retraining_status = {}
    activity_retraining_buffers = {}
    new_autoencoders = {}
    new_scalers = {}
    retraining_start_events = {}
    ...
    for event_idx, event_dict in enumerate(stream_csv_row_by_row(file_path)):
        try:
                ...
                if activity_label in activity_retraining_status:
                    if activity_retraining_status[activity_label] == "collecting_data":
                        ...
                        if events_collected >= min_buffer_to_train and events_collected % incremental_interval == 0:
                            ...
                            new_autoencoders[activity_label] = autoencoders.get(activity_label)
            ...
            if not _reported_first_event_error:
                import traceback
                traceback.print_exc()
                _reported_first_event_error = True
            event_processing_errors += 1
            continue
    ...
    if event_processing_errors:
        print(f"[COMPLETED] WARNING: {event_processing_errors} event(s) failed to process "
              f"and were skipped; metrics below cover only the events that succeeded")
...
def main():
    ...
    parser.add_argument('--seed', type=int,
                        default=int(os.environ.get('DUPLIMEND_SEED', 42)),
                        help='Random seed for this run. Overrides the DUPLIMEND_SEED '
                             'environment variable; defaults to 42. Vary this across '
                             'runs to reproduce the multi-seed experiments.')
    args = parser.parse_args()

    reset_global_state()
    force_cpu_mode = (args.mode == "offline_mode")
    set_all_seeds(args.seed, force_cpu=force_cpu_mode)
    ...
    activity_embeddings = DynamicActivityEmbeddings(embedding_dim=embedding_dim, seed=args.seed)
    ...
    save_config_to_output(cluster_tracker.output_dir, "main_execution", {"results_base_dir": results_base_dir, "tracking_dir": cluster_tracker.output_dir, "seed": args.seed})
